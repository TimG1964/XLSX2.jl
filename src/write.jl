...
A new `XLSXFile` created with `XLSX2.newxlsx` (or using `openxlsx` without specifying a filename) will 
...
function addsheet!(wb::Workbook, name::AbstractString=""; relocatable_data_path::String=_relocatable_data_path())::Worksheet
    ...
    new_cache = XLSX2.WorksheetCache(
        true,
        Dict{Int64,Dict{Int64,XLSX2.Cell}}(),
        Int64[],
...
See also [`XLSX2.openxlsx`](@ref) and [XLSX2.opentemplate](@ref).
...
# Examples
```julia
julia> f=XLSX2.openxlsx("general.xlsx", mode="rw")
...
julia> XLSX2.copysheet!(f[4])
12×8 XLSX2.Worksheet: ["table (copy)"](A2:H13)
...
julia> XLSX2.deletesheet!(f, "table5")
...
XLSX2.writetable("table.xlsx", columns, colnames)
```
