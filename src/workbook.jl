...
Writable XLSXFile instances are opened with `XLSX2.open_xlsx_template` method.
=#
...
function get_defined_names(f::XLSXFile, type::Symbol)
    if type == :Workbook
        names=keys(XLSX2.get_workbook(f).workbook_names)
        vals = values(XLSX2.get_workbook(f).workbook_names)
        list1 = ["\"Workbook\" $k =>  $(v.value)" for (k, v) in collect(zip(names, vals))]
    elseif type == :Worksheet 
        names=keys(XLSX2.get_workbook(f).worksheet_names)
        vals = values(XLSX2.get_workbook(f).worksheet_names)
...
In the context of `XLSX2.jl` there is no difference between an absolute reference and a relative 
...
# Examples
```julia
julia> XLSX2.addDefinedName(sh, "ID", "C21")

julia> XLSX2.addDefinedName(sh, "NEW", "A1:B2")

julia> XLSX2.addDefinedName(sh, "my_name", "A1,B2,C3")

julia> XLSX2.addDefinedName(xf, "New", "'Mock-up'!A1:B2")

julia> XLSX2.addDefinedName(xf, "Life_the_universe_and_everything", 42)

julia> XLSX2.addDefinedName(xf, "first_name", "Hello World")
