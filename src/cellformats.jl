...
# Examples:
```julia
julia> XLSX2.setFormat(sh, "D2"; format = "h:mm AM/PM")

julia> XLSX2.setFormat(xf, "Sheet1!A2"; format = "# ??/??")

julia> XLSX2.setFormat(sh, "F1:F5"; format = "Currency")

julia> XLSX2.setFormat(sh, "named_range"; format = "Percentage")

julia> XLSX2.setFormat(sh, "A2"; format = "_-£* #,##0.00_-;-£* #,##0.00_-;_-£* \\\"-\\\"??_-;_-@_-")
...
# Examples:
```julia
julia> XLSX2.setUniformFormat(xf, "Sheet1!A2:L6"; format = "# ??/??")

julia> XLSX2.setUniformFormat(sh, "F1:F5"; format = "Currency")
...
# Examples:
```julia
julia> XLSX2.setUniformStyle(xf, "Sheet1!A2:L6")

julia> XLSX2.setUniformStyle(sh, "F1:F5")

julia> XLSX2.setUniformStyle(sh, 2:5, 5)

julia> XLSX2.setUniformStyle(sh, 2, :)
...
# Examples:
```julia
julia> XLSX2.setColumnWidth(xf, "Sheet1!A2"; width = 50)

julia> XLSX2.seColumnWidth(sh, "F1:F5"; width = 0)

julia> XLSX2.setColumnWidth(sh, "I"; width = 24.37)
...
# Examples:
```julia
julia> XLSX2.getColumnWidth(xf, "Sheet1!A2")

julia> XLSX2.getColumnWidth(sh, "F1")

julia> XLSX2.getColumnWidth(sh, 1, 6)
...
# Examples:
```julia
julia> XLSX2.setRowHeight(xf, "Sheet1!A2"; height = 50)

julia> XLSX2.setRowHeight(sh, "F1:F5"; height = 0)

julia> XLSX2.setRowHeight(sh, "I"; height = 24.56)
...
# Examples:
```julia
julia> XLSX2.getRowHeight(xf, "Sheet1!A2")

julia> XLSX2.getRowHeight(sh, "F1")

julia> XLSX2.getRowHeight(sh, 1, 6)
...
# Examples:
```julia
julia> f = XLSX2.readxlsx("test.xlsx")
...
julia> s = f["Sheet1"]
2×2 XLSX2.Worksheet: ["Sheet1"](A1:B2)

julia> XLSX2.getMergedCells(s)
1-element Vector{XLSX2.CellRange}:
 B1:B2
...
# Examples:
```julia
julia> XLSX2.isMergedCell(xf, "Sheet1!A1")

julia> XLSX2.isMergedCell(sh, "A1")

julia> XLSX2.isMergedCell(sh, 2, 4) # cell D2

julia> mc = XLSX2.getMergedCells(sh)

julia> XLSX2.isMergedCell(sh, XLSX2.CellRef("A1"), mc)
...
# Examples:
```julia
julia> XLSX2.getMergedBaseCell(xf, "Sheet1!B2")
(baseCell = B1, baseValue = 3)

julia> XLSX2.getMergedBaseCell(sh, "B2")
(baseCell = B1, baseValue = 3)

julia> XLSX2.getMergedBaseCell(sh, 2, 2)
(baseCell = B1, baseValue = 3)
...
# Examples:
```julia
julia> XLSX2.mergeCells(xf, "Sheet1!B2:D3")  # Merge a cell range.

julia> XLSX2.mergeCells(sh, 1:3, :)          # Merge rows to the extent of the dimension.

julia> XLSX2.mergeCells(sh, "A:D")           # Merge columns to the extent of the dimension.
