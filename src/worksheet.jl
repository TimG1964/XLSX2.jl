...
```julia
julia> f = XLSX2.readxlsx("myfile.xlsx")
...
julia> cell = XLSX2.getcell(sheet, "A1")

julia> cell = XLSX2.getcell(sheet, 1:3, [2,4,6])
...
julia>  XLSX2.getcellrange(f[1], ncr)
3-element Vector{Matrix{XLSX2.AbstractCell}}:
 [XLSX2.Cell(B3, "", "", "5", XLSX2.Formula("", nothing));;]
 [XLSX2.Cell(A1, "", "", "2", XLSX2.Formula("", nothing));;]
 [XLSX2.Cell(C2, "", "", "5", XLSX2.Formula("", nothing));;]
