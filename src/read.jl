...
and saving to another file with [XLSX2.writexlsx](@ref).

A convenience function equivalent to `openxlsx(source; mode="rw", enable_cache=true)`

!!! note
    XLSX2.jl only works with `.xlsx` files and cannot work with Excel `.xltx` template files. 
    Reading as a template in this package merely means opening a `.xlsx` file to edit, update 
    and then write as an updated `.xlsx` file (e.g. using `XLSX2.writexlsx()`). Doing so retains 
...
function _relocatable_data_path(; path::AbstractString=Artifacts.artifact"XLSX2_relocatable_data")
    return path
end
...
subsequent saving to a file with [XLSX2.writexlsx](@ref).
By default, the worksheet is `Sheet1`. Specify `sheetname` to give the worksheet a different name.

# Examples
```julia
julia> xf = XLSX2.newxlsx()

julia> xf = XLSX2.newxlsx("MySheet")
```
...
Consider using [`XLSX2.openxlsx`](@ref) for lazy loading of Excel file contents.
"""
...
```julia
XLSX2.openxlsx("myfile.xlsx") do xf
    # read data from `xf`
end
```
...
!!! warning
    ...
    The `rw` mode is known occasionally to produce some data loss. See [#159](https://github.com/felipenoris/XLSX2.jl/issues/159). (Now fixed!)
...
```julia
julia> XLSX2.openxlsx("myfile.xlsx", enable_cache=false) do xf
...
```julia
XLSX2.openxlsx("new.xlsx", mode="w") do xf
    sheet = xf[1]
...
```julia
XLSX2.openxlsx("edit.xlsx", mode="rw") do xf
    sheet = xf[1]
    sheet[2, :] = [2, Date(2019, 1, 1), "add new line"]
end
```

See also [`XLSX2.readxlsx`](@ref).
"""
...
```
XLSX2.openxlsx("myfile.xlsx", mode="rw") do xf
...
function check_minimum_requirements(xf::XLSXFile)
    ...
    elseif occursin("spreadsheetml.template", content_types)
        throw(XLSXError("XLSX2.jl does not support Excel template files (`.xltx` files).\nSave template as an `xlsx` file type first."))
    else
...
```julia
julia> XLSX2.readdata("myfile.xlsx", "mysheet", "A2:B4")
3×2 Array{Any,2}:
 1  "first"
 2  "second"
 3  "third"

julia> XLSX2.readdata("myfile.xlsx", 1, "A2:B4")
3×2 Array{Any,2}:
 1  "first"
 2  "second"
 3  "third"
...
```julia
julia> XLSX2.readdata("customXml.xlsx", "Mock-up", "Location") # `Location` is a `definedName` for a non-contiguous range
...
Returns tabular data from a spreadsheet as a struct `XLSX2.DataTable`.
...
julia> df = DataFrame(XLSX2.readtable("myfile.xlsx", "mysheet"))
```

See also: [`XLSX2.gettable`](@ref).
"""
...
Takes the same keyword arguments as [`XLSX2.readtable`](@ref) 
...
julia> df = XLSX2.readto("myfile.xlsx", DataFrame)

julia> df = XLSX2.readto("myfile.xlsx", StructArray)

julia> df = XLSX2.readto("myfile.xlsx", "mysheet", DataFrame)

julia> df = XLSX2.readto("myfile.xlsx", "mysheet", "A:C", DataFrame)
```
...
function readto(source::Union{AbstractString,IO}, sheet::Union{AbstractString,Int}, range::AbstractString, sink=nothing; kw...)
    if sink === nothing
        throw(XLSXError("provide a valid sink argument, like `using DataFrames; XLSX2.readdf(source, sheet, columns, DataFrame)`"))
    end
...
function readto(source::Union{AbstractString,IO}, sheet::Union{AbstractString,Int}, sink=nothing; kw...)
    if sink === nothing
        throw(XLSXError("provide a valid sink argument, like `using DataFrames; XLSX2.readdf(source, sheet, DataFrame)`"))
    end
