...
First, make sure you have **XLSX2.jl** package installed.
...
julia> xf = XLSX2.readxlsx("myfile.xlsx")
...
4×2 XLSX2.Worksheet: ["mysheet"](A1:B4)
...
julia> XLSX2.readdata("myfile.xlsx", "mysheet", "A2:B4") # shorthand for all above
3×2 Array{Any,2}:
 1  "first"
 2  "second"
 3  "third"
...
The example above used `xf = XLSX2.readxlsx(filename)` to open a file, so all file contents are fetched at once from disk.

You can also use `XLSX2.openxlsx` to read file contents as needed (see [Reading Large Excel Files and Caching](@ref)).
...
The [`XLSX2.gettable`](@ref) method returns tabular data from a spreadsheet as a struct `XLSX2.DataTable`
...
There are also two helper methods [`XLSX2.readtable`](@ref) and [`XLSX2.readto`](@ref) to read from file 
directly, as shown in the following examples.

```julia
julia> using DataFrames, XLSX

julia> df = DataFrame(XLSX2.readtable("myfile.xlsx", "mysheet")) # Returns a `Tables.jl` table that `DataFrame` can accept
3×2 DataFrame
 Row │ HeaderA  HeaderB 
     │ Int64    String  
─────┼──────────────────
...
julia> df = XLSX2.readto("myfile.xlsx", "mysheet", DataFrame) # Returns a `DataFrame` directly.
3×2 DataFrame
 Row │ HeaderA  HeaderB 
     │ Int64    String  
─────┼──────────────────
...
Use [`XLSX2.readdata`](@ref) or [`XLSX2.getdata`](@ref) to read content as a Julia matrix.

```julia
julia> import XLSX

julia> m = XLSX2.readdata("myfile.xlsx", "mysheet!A1:B3")
3×2 Array{Any,2}:
  "HeaderA"  "HeaderB"
 1           "first"
 2           "second"
...
Indexing in a `Worksheet` will dispatch to [`XLSX2.getdata`](@ref) method.
...
[`XLSX2.readtable`](@ref) and [`XLSX2.gettable`](@ref) methods.

```julia
julia> dtable = XLSX2.readtable("myfile.xlsx", "mysheet")
XLSX2.DataTable(Any[Any[1, 2, 3], Any["first", "second", "third"]], [:HeaderA, :HeaderB], Dict(:HeaderB => 2, :HeaderA => 1))
...
The method `XLSX2.openxlsx` has a `enable_cache` option to control worksheet cells caching.
...
and `openxlsx` is used with do-syntax, the xlsx file itself will be opened using `Mmap.mmap` so that the 
...
```julia
julia> XLSX2.openxlsx("myfile.xlsx", enable_cache=false) do f
           sheet = f["mysheet"]
           for r in XLSX2.eachrow(sheet)
              # r is a `SheetRow`, values are read using column references
              rn = XLSX2.row_number(r) # `SheetRow` row number
...
You could also stream tabular data using `XLSX2.eachtablerow(sheet)`, which is the underlying iterator in `gettable` method.
Check docstrings for `XLSX2.eachtablerow` for more advanced options.

```julia
julia> XLSX2.openxlsx("myfile.xlsx", enable_cache=false) do f
           sheet = f["mysheet"]
           for r in XLSX2.eachtablerow(sheet)
               # r is a `TableRow`, values are read using column labels or numbers
               rn = XLSX2.row_number(r) # `TableRow` row number
...
Opening a file in `write` mode with `XLSX2.openxlsx` will open a new (blank) Excel file for editing.

```julia
XLSX2.openxlsx("my_new_file.xlsx", mode="w") do xf
    sheet = xf[1]
    XLSX2.rename!(sheet, "new_sheet")
    sheet["A1"] = "this"
    sheet["A2"] = "is a"
    sheet["A3"] = "new file"
    sheet["A4"] = 100
...
Opening a file in `read-write` mode with `XLSX2.openxlsx` will open an existing Excel file for editing.
This will preserve existing data and formatting in the original file.

```julia
XLSX2.openxlsx("my_new_file.xlsx", mode="rw") do xf
    sheet = xf[1]
    sheet["B1"] = "new data"
end
```
...
Given a sheet reference, use the `XLSX2.writetable!` method. Anchor cell defaults to cell `"A1"`.
...
You can also use `XLSX2.writetable` to write directly to a new file (see next section).

### Export Tabular Data from any `Tables.jl` compatible source

To export tabular data to Excel, use `XLSX2.writetable` method, which accepts either columns and column names,
...
julia> XLSX2.writetable("df.xlsx", df)
```
...
julia> XLSX2.writetable("report.xlsx", "REPORT_A" => df1, "REPORT_B" => df2)
```
...
XLSX2.openxlsx("report.xlsx", mode="w") do xf
    for i in eachindex(sheet_names)
        ...
        if i == firstindex(sheet_names)
            sheet = xf[1]
            XLSX2.rename!(sheet, sheet_name)
            XLSX2.writetable!(sheet, df)
        else
            sheet = XLSX2.addsheet!(xf, sheet_name)
            XLSX2.writetable!(sheet, df)        
        end
    end
end
...
Both types `XLSX2.DataTable` and `XLSX2.TableRowIterator` conforms to [Tables.jl](https://github.com/JuliaData/Tables.jl) interface.
An instance of `XLSX2.TableRowIterator` is created by the function `XLSX2.eachtablerow`.

Also, `XLSX2.writetable` accepts an argument that conforms to the `Tables.jl` interface.
...
julia> XLSX2.writetable("output_table.xlsx", df, overwrite=true, sheetname="report", anchor_cell="B2")

julia> f = XLSX2.readxlsx("output_table.xlsx")
...
julia> s = f["report"]
6×7 XLSX2.Worksheet: ["report"](A1:G6)

julia> df2 = XLSX2.eachtablerow(s) |> DataFrames.DataFrame
4×6 DataFrames.DataFrame
│ Row │ integers │ strings │ floats  │ dates      │ times    │ datetimes           │
│     │ Int64    │ String  │ Float64 │ Date       │ Time     │ DateTime            │
├─────┼──────────┼─────────┼─────────┼────────────┼──────────┼─────────────────────┤
